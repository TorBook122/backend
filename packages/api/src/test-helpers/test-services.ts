...
import notificationsApp from '@torbook/notifications/server';
...
export async function startTestServices(): Promise<void> {
  ...
  const [dbPort, authPort, sharedPort, queuePort, notificationsPort] = await Promise.all([
    listen(dbApp),
    listen(authApp),
    listen(sharedApp),
    listen(queueApp),
    listen(notificationsApp),
  ]);
  ...
  process.env.NOTIFICATIONS_SERVICE_URL = `http://127.0.0.1:${notificationsPort}`;
